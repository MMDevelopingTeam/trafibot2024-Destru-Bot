...
const { launchBotsSendMessage } = require("../utils/launchBotSendMessage");
...
const BotSendMessage = async(req, res) => {
  const {token} = req.body;
  let dataLaunch = null;
  ...
  async function main() {
    ...
    try {
      ...
      let message = "promoviendo a lolita jejes";

      const dataAcct = await accountsModels.findOne({isUsed: false})
      if (!dataAcct) return res.status(400).send({
          success: false,
          message: 'No hay cuentas libres'
      });
      const dataProxy = await proxysModels.findOne({isFull: false}).sort({ms: 1})
      if (!dataProxy) return res.status(400).send({
          success: false,
          message: 'No hay proxys libres'
      });
      dataProxy.Nusers++
      if (dataProxy.Nusers === 10) dataProxy.isFull = true
      await dataProxy.save();
      if (dataProxy && dataProxy.isDown === false) {
        const dataProxyIdPackage = await idPackProxyModels.findOne({_id: dataProxy.idPackage})
        if (!dataProxyIdPackage) {
        ...
        if (dataProxyIdPackage.platform === "instantproxies") {
          launchBotsSendMessage(proxy, id, username, password, index, idRegisterCompBotContainer, message)
        }
        if (dataProxyIdPackage.platform === "otros") {
          //console.log("hola")
          //proxy + id
          //console.log(dataProxy.proxy + dataAcct._id+ dataAcct.username+ dataAcct.password+ "index"+ dataLaunch.idRegisterCompBotContainer+ message)
          //console.log(proxy, id, username, password, index, idRegisterCompBotContainer, message)
          launchBotsSendMessage(dataProxy.proxy, dataAcct._id, dataAcct.username, dataAcct.password, 1, dataLaunch.idRegisterCompBotContainer, message)
          //launchBotColor(dataProxy.proxy, dataAcct._id, dataLaunch.nameModel, dataAcct.username, dataAcct.password, indexAcc, dataLaunch.idRegisterCompBotContainer, isFollow)
        }
      }
      dataAcct.isUsed = true
...
module.exports = {getBot, botDebugControl, killBot, getBotAny, killBotAny, VerifyBotskillBot, getBotMixed, killBotMixed, BotFollowers, killBotFollowers, BotSendMessage, BotColor, killBotColor, status};